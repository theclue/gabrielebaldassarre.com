...
excerpt: In questo articolo introdurremo concettualmente le reti _small-world_ e il modello Watts-Strogatz creato per studiare i fenomeni sociali che fanno sì che gli esseri umani siano tra loro separati dai famosi sei gradi di separazione.
...
* \\( 250^2 = `r format(250^2, scientific = FALSE, big.mark=".", decimal.mark = ",") ` \\) individui a due passi di distanza da me;
* \\( 250^3 = `r format(250^3, scientific = FALSE, big.mark=".", decimal.mark = ",") ` \\) individui a tre passi di distanza da me.
* ...
...
```{r watts-strogatz.ring, warning = FALSE, fig.asp = 1}

par(mfrow=c(1,1))

simplest.ring.network <- sample_smallworld(1, 20, 0, 0)

l <- layout_in_circle(simplest.ring.network)

plot(simplest.ring.network,
     vertex.size = vertex.size,
     vertex.color = vertex.color,
     vertex.label = NA)
mtext("n = 20, k = 2", side = 1, line = 0, cex = 1.2)
...
```{r watts-strogatz-randomness, fig.asp=1, warning=FALSE}
...
l.polar <- cart2pol(l.df$x, l.df$y) %>%
  mutate(r = ifelse(row_number() %% 2 == 1, r - .1, r))

l <- as.matrix(pol2cart(l.polar$r, l.polar$theta) %>% select(x, y))


smallworld.network <- sample_smallworld(1, 20, 2, 0.1)
smallworld.network.2 <- sample_smallworld(1, 20, 2, 0.3)
...
plot(ring.network,
     ...
     layout = l,
     edge.curved = .1)
mtext("p = 0", side = 1, line = 0, cex = 1.2)
...
plot(smallworld.network,
     ...
     layout = l,
     edge.curved = .1)
mtext("p = 0.1", side = 1, line = 0, cex = 1.2)
title(main="Rete Riconnessa 1", cex.main = 1.5)

plot(smallworld.network.2,
     vertex.size = vertex.size,
     vertex.color = vertex.color,
     vertex.label = NA,
     layout = l,
     edge.curved = .1)
mtext("p = 0.3", side = 1, line = 0, cex = 1.2)
title(main="Rete Riconnessa 2", cex.main = 1.5)
...
plot(random.network,
     ...
     layout = l,
     edge.curved = .1)
mtext("p = 1", side = 1, line = 0, cex = 1.2)
...
In queste reti la distribuzione del grado (_degree distribution_) è, piuttosto, assimilabile a una [legge di potenza]({% post_url 2018-08-12-legge-potenza-powerlaw %}). Modelli più recenti di descrizione delle reti, come il modello [Barabási–Albert](https://en.wikipedia.org/wiki/Barabási–Albert_model) tengono conto di questo comportamento asimmetrico dei nodi.

D'altra parte, né il modello Erdős-Renyi né il modello Barabási–Albert simulano correttamente, al contrario del modello Watts-Strogatz, la coesione locale. Nessuno di essi, in definitiva, può considerarsi un modello realistico per descrivere la struttura di una rete sociale, ma solo una approssimazione.

{% figure caption:"La rete individuata dagli aeroporti e dalle rotte degli aerei è un'altra rete _small-world_ molto studiata, ma che bene mette in evidenza i limiti del modello Watts-Strogatz sull'incapacità di distinguere i nodi per importanza. È evidente, infatti, come al Chicago International Airport confluiscano centinaia se non migliaia di rotte da molti altri scali e questo ne fa innegabilmente un _hub_, di gran lunga più importante dei molti scali regionali disseminati in tutti gli Stati Uniti. Questa caratteristica delle reti _small-world_ è molto importante; basti pensare alle ripercussioni che si avrebbe sull'intero traffico aereo se un _hub_ dovesse essere non operativo, di gran lunga più serie di ciò che si avrebbe con uno scalo minore. Non a caso si dice che le reti _small-world_ sono molto vulnerabili ad attacchi mirati." %}
