...
```{r vectors.dotproduct, fig.asp= .6, warning= FALSE}
a <- c(1,3)
...
$$ \bbox[5px,border:1px solid red]{ AV = V\Lambda } $$
...
```{r eigen.example, include = FALSE}
eigen.example.values <- c(2,-1,3,3,-2,2,15,-9,16)
eigen.mat <- matrix(eigen.example.values, nrow = 3, byrow = T)
ev <- eigen(eigen.mat)
```
...
$$ \begin{matrix} \lambda_1 = `r format(ev$values[1], digits = 2, nsmall = 2)` \\\\ \lambda_2 = `r format(ev$values[2], digits = 2, nsmall = 2)` \\\\ \lambda_3 = `r format(ev$values[3], digits = 2, nsmall = 2)` \end{matrix} $$
