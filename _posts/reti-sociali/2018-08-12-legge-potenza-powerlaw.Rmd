...
---
```{r powerlaw.setup, include = FALSE, cache = FALSE}
if (!require("pacman")) install.packages("pacman"); invisible(library(pacman))
tryCatch({
  p_load("tidyverse", "poweRlaw", "rprojroot")
}, warning=function(w){
...
```{r italian.towns}
italian.towns <- read.csv2(find_root_file("R", "data", "ISTAT", "comuni_italiani.csv", criterion = is_git_root))
...
Così, per essere originale, ho ben pensato di studiare il dataset dei comuni italiani, messo a disposizione dall'ISTAT a valle dell'ultimo censimento nazionale. Si tratta di un dataset di `r nrow(italian.towns) ` comuni italiani con una popolazione compresa tra `r min(italian.towns$PopResidente)` e `r max(italian.towns$PopResidente)` abitanti.
